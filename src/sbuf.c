...
static bool tls_change_requires_reconnect(struct tls_config *new_server_connect_conf)
{
	if (server_connect_sslmode != cf_server_tls_sslmode) {
		...
		return true;
		/* This function we've added to libusual does not implement checking tls_conf->keypair not
		 * checked because it is nulled out at this point for both the new_server_connect_conf
		 * and the * original server_connect_conf. There is nothing to check */
