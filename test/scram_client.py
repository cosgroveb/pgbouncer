...
class ScramClient:
    ...
    def _read_message(self):
        ...
        message_type, length = struct.unpack("!cI", header)
        if length < 4:
    ...
    def _send_startup(self):
        params = b"\0".join(
            (b"user", self.user.encode(), b"database", self.database.encode(), b"", b"")
        )
    ...
    def tls_server_end_point(self):
        ...
        # Integration certificates use SHA-256; C tests cover algorithm edge cases.
    ...
    def finish(
        ...
        client_final_without_proof = (
            b"c=" + channel_binding + b",r=" + self.server_nonce + extensions
        )
        auth_message = b",".join(
